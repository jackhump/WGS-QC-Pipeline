...
#create empty vectors that will get the values of significant missingness chromosomes.
pass1 <- c()
pass2 <- c()

#Filters missingness file based on threshold, and fills the two vectors from above. Does not consider duplicate sample combinations
...
#remove samples that are related to the most number of other samples. Repeat is a looped search to remove these.
removal <- c()
...
repeat{
  ...
  #maxVal <- names(occurances)[grep(max(occurances),occurances)[rand(length(grep(max(occurances),occurances)))]]
